...
impl Model {
    ...
    pub fn calc_grad(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let gauss = param.apply("gauss", f64::exp, f64::exp);
        ...
        for pt in &mut self.points {
            ...
            let val = gauss.eval();
            gauss.backprop()?;
            println!("Stats for point {}", pt.pos);
            println!("val: {:?}", val);
            println!("mu_x: {:?}", mu_x.grad());
            println!("mu_x: {:?}", mu_y.grad());
    ...
    pub fn plot_sigma(&self, radius: f64, mut f: impl FnMut(Vec2<f64>)) {
        ...
        for i in 0..=36 {
            ...
            let tr_x = x * sigma_inv00 + y * sigma_inv10 + self.gauss.mu.x;
            let tr_y = x * sigma_inv01 + y * sigma_inv11 + self.gauss.mu.y;
